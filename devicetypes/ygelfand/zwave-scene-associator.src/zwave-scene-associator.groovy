 ...
 */
 ...
 metadata {
	...
	definition (name: "Z-Wave Scene Associator", namespace: "ygelfand", author: "ygelfand") {
		capability "Actuator"
        capability "Button"
...
def getparamState() {
    ...
    def cmds = []
    //cmds << zwave.multiInstanceV1.multiInstanceCmdEncap(command:1, commandClass:0x91, instance:1, payload: [15, 38, 1, 2, 50, 0]).format()
    sendHubCommand(new physicalgraph.device.HubAction("91001D0D01FF01180508D3"))
    log.debug "sending $cmds --- ${device.deviceNetworkId}"
    setLightStatus(true, true, false,false)
